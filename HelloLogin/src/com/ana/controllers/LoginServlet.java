...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (existeUser) {
			session.setAttribute("usuario", emailrecibido); 	//se genera un flag para el resto de peticiones, es decir, para mostrar al resto de páginas que se tiene sesión abierta
			response.sendRedirect("listausuarios");
		}else {
			request.setAttribute("error", "Usuario y contraseña no son correcto.");
...
//			response.sendRedirect("listausuarios");
//			//se realiza el cambio de dirección en el navegador y se muestra para el usuario
//			
//		}else {
